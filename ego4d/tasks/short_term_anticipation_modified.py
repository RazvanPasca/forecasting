...
class ShortTermAnticipationTask(VideoTask):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # model forward pass
        detections, raw_detections = self.model.forward(
            inputs,
            pred_boxes,
            extra_data["orig_pred_boxes"],
            extra_data["pred_object_labels"],
        ...
        return {"uids": uids, "pred_detections": detections, "raw_detections": raw_detections}
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.global_rank == 0:
            res = {
                ...
                #         {
                #             "box": [float(h) for h in z[0]],
                #             "object_labels": int(z[1]),
                #             "object_scores": float(z[2]),
                #             "time_to_contact": float(z[3][0]),
                #             "verb_scores": [float(x) for x in (np.around(z[4], 3))],
                #         }
                #         for z in zip(x["boxes"], x["object_labels"], x["object_scores"], x["ttcs"], x["verb_scores"])
                #     ]
                #     for uid, x in zip(data["uids"], data["raw_detections"])
                # },
            }
