...
import numpy as np
from Code.data_preprocessing.utils.annotations_df_utils import MAX_REC_EXTRACT_OFFSET
from Ego4d_all.forecast.ego4d.datasets.short_term_anticipation import Ego4DHLMDB, PyAVVideoReader
from torch.utils.data import DataLoader, Dataset
from tqdm import tqdm

parser = ArgumentParser()

parser.add_argument("--path_to_annotations", type=Path, default=os.path.expandvars("$DATA/Ego4d/data/annotations"))
parser.add_argument("--path_to_videos", type=Path, default=os.path.expandvars("$DATA/Ego4d/data/full_scale"))
parser.add_argument("--path_to_output_lmdbs", type=Path, default=os.path.expandvars("$DATA/Ego4d/data/lmdb"))
parser.add_argument("--batch_size", type=int, default=2)
parser.add_argument("--context_frames", type=int, default=40)
...
class PyAVSTADataset(Dataset):
    def __init__(self, annotations, path_to_videos, existing_keys=None, fps=30, max_chunk_size=32, retry=10):
        print(
        ...
        for ann in annotations:
            ...
            mid_frame = ann["frame"]
            try:
                this_fps = train["info"]["video_metadata"][video_id]["fps"]
            except:
                this_fps = val["info"]["video_metadata"][video_id]["fps"]

            last_frame = mid_frame + int(ann["objects"][0]["time_to_contact"] * this_fps) + MAX_REC_EXTRACT_OFFSET
            first_frame = np.max([0, last_frame - args.context_frames + 1])
            frame_numbers = np.arange(min(first_frame, mid_frame), last_frame + 1)
        ...
        for k, v in frames_per_video.items():
            frames = np.setdiff1d(np.sort(np.unique(v)), existing_frames.get(k, []))
...
dset = PyAVSTADataset(annotations, args.path_to_videos, existing_keys=None)
dloader = DataLoader(dset, batch_size=args.batch_size, collate_fn=collate, num_workers=12, prefetch_factor=1)
